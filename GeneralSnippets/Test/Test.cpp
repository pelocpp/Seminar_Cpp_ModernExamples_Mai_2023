...
namespace VariadicGenericFolding_16 {
    ...
    auto combine(const auto& binaryFunc, auto pred1, auto pred2)
    {
        return [=](auto param) {
            return binaryFunc(pred1(param), pred2(param));
        };
    };
    ...
    auto boolAnd = [](auto left, auto right) {
        return left && right;
    };
